...
int main(void)
{
    ...
    P1OUT |= BIT1; // switch dir
    P1REN |= BIT1; // switch pullup

    P1IE |= BIT1; // Interrupt enable
    P1IES |= BIT1; // Interupt edge(Falling Edge)
    P1IFG &= ~BIT1; // Interupt Flag
...
int main(void)
{
    ...
    while(1)
    {
        if(global_count>=1000){
            ...
            if(mode_select == 1){
                if(led_state == 0){
                    P1OUT |= BIT0; // left LED 켜기
                    P4OUT |= BIT7; // right LED 켜기
                    led_state = 1;
                }else{
                    P1OUT &= ~BIT0; // left LED 끄기
                    P4OUT &= ~BIT7; // right LED 끄기
                    led_state = 0;
                }
